...
export default async function LojaPublicaPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: empresa } = await supabaseAdmin
    .from('empresas')
    .select('id, nome, telefone, whatsapp, instagram, cidade, estado, catalogo_cor_primaria, catalogo_cor_secundaria, catalogo_descricao, catalogo_template, catalogo_fonte, catalogo_logo_url, catalogo_mostrar_carrinho')
    .eq('slug', slug)
    .single()
