...
type ClienteInativo = {
  ...
  diasSemComprar: number; totalGasto: number; numCompras: number
  categoria: 'atencao'|'sumido'|'perdido'
}
...
const cats = {
  atencao: { emoji:'🟡', label:'Atenção',  desc:'Quase sumindo...',   cor:'var(--amarelo)' },
  sumido:  { emoji:'🟠', label:'Sumido',   desc:'Passou do prazo',    cor:'#c05200' },
  perdido: { emoji:'🔴', label:'Perdido',  desc:'Muito tempo sumido', cor:'var(--vermelho)' },
}

function msgWhatsApp(nome: string, categoria: string, nomeEmpresa: string) {
  const first = nome.split(' ')[0]
  if (categoria === 'perdido')
    return `Olá ${first}! Faz um tempo que não te vemos por aqui na ${nomeEmpresa}... Tenho novidades que você vai adorar. Passa na loja ou me chama aqui! 😊`
  if (categoria === 'sumido')
    return `Olá ${first}! Tô com novidades aqui na ${nomeEmpresa} e lembrei de você. Vem dar uma olhada! 💪`
  return `Oi ${first}! Novidade boa chegou na ${nomeEmpresa}. Pode vir conferir? 🙌`
}
...
export default function ClientesInativosPage() {
  ...
  const [config,   setConfig]   = useState<CrmConfig>({ diasAtencao:30, diasSumido:60, diasPerdido:90, nomeEmpresa:'nossa loja' })
  ...
  async function carregar(eid: string) {
    ...
    // Busca config CRM da empresa
    const { data: emp } = await supabase
      .from('empresas')
      .select('crm_dias_atencao, crm_dias_sumido, crm_dias_perdido, crm_prazo_inatividade_dias, nome')
      .eq('id', eid)
      .single()

    const diasAtencao = emp?.crm_dias_atencao ?? Math.max(15, Math.floor((emp?.crm_prazo_inatividade_dias ?? 60) / 2))
    const diasSumido  = emp?.crm_dias_sumido  ?? (emp?.crm_prazo_inatividade_dias ?? 60)
    const diasPerdido = emp?.crm_dias_perdido ?? Math.round((emp?.crm_prazo_inatividade_dias ?? 60) * 1.5)
    const nomeEmpresa = emp?.nome ?? 'nossa loja'

    setConfig({ diasAtencao, diasSumido, diasPerdido, nomeEmpresa })
    ...
    await Promise.all(clientes.map(async (c) => {
      ...
      // Só inclui clientes que atingiram pelo menos o limiar de atenção
      if (dias < diasAtencao) return
      ...
      const categoria: 'atencao'|'sumido'|'perdido' =
        dias >= diasPerdido ? 'perdido' : dias >= diasSumido ? 'sumido' : 'atencao'

      inatArray.push({
        id: c.id, nome: c.nome, telefone: c.telefone,
        ultima_compra: c.ultima_compra, diasSemComprar: dias,
        totalGasto, numCompras, categoria
      })
    }))
  ...
  const qtdAtencao = inativos.filter(c => c.categoria === 'atencao').length
  const qtdSumido  = inativos.filter(c => c.categoria === 'sumido').length
  const qtdPerdido = inativos.filter(c => c.categoria === 'perdido').length
  const potencial  = inativos
    .filter(c => c.categoria !== 'atencao')
  ...
  return (
    ...
    <div className="anim-fade" style={{display:'flex',flexDirection:'column',gap:'0.875rem'}}>
      ...
      {/* Info dos limiares configurados */}
      <div style={{ display:'flex', gap:'0.375rem', flexWrap:'wrap' }}>
        {([
          { cat:'atencao', label:`🟡 Atenção ≥ ${config.diasAtencao}d` },
          { cat:'sumido',  label:`🟠 Sumido ≥ ${config.diasSumido}d`  },
          { cat:'perdido', label:`🔴 Perdido ≥ ${config.diasPerdido}d` },
        ] as const).map(({ label }) => (
          <span key={label} style={{ fontSize:'0.68rem', padding:'0.2rem 0.5rem', background:'var(--surface-alt)', border:'1px solid var(--borda)', borderRadius:'var(--radius-sm)', color:'var(--texto-sec)', fontWeight:600 }}>
            {label}
          </span>
        ))}
        <a href="/configuracoes" style={{ fontSize:'0.68rem', padding:'0.2rem 0.5rem', color:'var(--texto-desab)', textDecoration:'underline', alignSelf:'center' }}>
          ⚙ Ajustar
        </a>
      </div>
      ...
      {loading ? (
        ...
        <div style={{textAlign:'center',padding:'3rem',color:'var(--texto-desab)'}}>
          ...
          <p style={{fontSize:'0.85rem',marginTop:'0.25rem'}}>
            Todos os seus clientes compraram nos últimos {config.diasAtencao} dias.
          </p>
        </div>
      ) : (
        <>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:'0.625rem'}}>
            {[
              {...cats.atencao, qtd:qtdAtencao, extra:`Alertar após ${config.diasAtencao}d`},
              {...cats.sumido,  qtd:qtdSumido,  extra:`Alertar após ${config.diasSumido}d`},
              {...cats.perdido, qtd:qtdPerdido, extra:`Alertar após ${config.diasPerdido}d`},
            ].map(c => (
            ...
            <div className="card" style={{padding:'0.875rem',borderLeft:'4px solid var(--verde)'}}>
              ...
              <p style={{fontWeight:900,fontSize:'1.4rem',color:'var(--verde)',fontFamily:'monospace'}}>
                {formatCurrency(potencial / ((qtdSumido + qtdPerdido) || 1))}
              </p>
            ...
            <table className="tabela">
              ...
              <tbody>
                {inativos.map(c => {
                  ...
                  const msg = encodeURIComponent(msgWhatsApp(c.nome, c.categoria, config.nomeEmpresa))
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'0.625rem'}}>
              {Object.entries(cats).map(([key,cat]) => (
                <div key={key} style={{padding:'0.625rem',background:'var(--surface-alt)',borderRadius:'var(--radius-sm)',borderLeft:`3px solid ${cat.cor}`}}>
                  ...
                  <p style={{fontSize:'0.82rem',color:'var(--texto-sec)',fontStyle:'italic'}}>"{msgWhatsApp('Cliente', key, config.nomeEmpresa)}"</p>
                </div>
              ))}
            </div>
          </div>
